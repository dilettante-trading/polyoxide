...
fn parse_leaderboard_category(s: &str) -> PyResult<polyoxide_data::api::leaderboard::LeaderboardCategory> {
...
fn parse_leaderboard_order_by(s: &str) -> PyResult<polyoxide_data::api::leaderboard::LeaderboardOrderBy> {
...
impl PyDataApiUser {
    ...
    fn list_positions<'py>(
        &self, py: Python<'py>,
        market: Option<Vec<String>>, event_id: Option<Vec<String>>,
        size_threshold: Option<f64>, redeemable: Option<bool>,
        mergeable: Option<bool>, limit: Option<u32>, offset: Option<u32>,
        sort_by: Option<String>, sort_direction: Option<String>,
        title: Option<String>,
    ) -> PyResult<Bound<'py, PyAny>> {
        let client = self.client.clone();
        let address = self.address.clone();
        let sort_by = sort_by.map(|s| parse_position_sort_by(&s)).transpose()?;
        let sort_direction = sort_direction.map(|s| parse_sort_direction(&s)).transpose()?;
        pyo3_async_runtimes::tokio::future_into_py(py, async move {
            let mut req = client.user(&address).list_positions();
            if let Some(v) = market { req = req.market(v); }
            if let Some(v) = event_id { req = req.event_id(v); }
            if let Some(v) = size_threshold { req = req.size_threshold(v); }
            if let Some(v) = redeemable { req = req.redeemable(v); }
            if let Some(v) = mergeable { req = req.mergeable(v); }
            if let Some(v) = limit { req = req.limit(v); }
            if let Some(v) = offset { req = req.offset(v); }
            if let Some(v) = sort_by { req = req.sort_by(v); }
            if let Some(v) = sort_direction { req = req.sort_direction(v); }
            if let Some(v) = title { req = req.title(v); }
    ...
    fn positions_value<'py>(
        &self, py: Python<'py>, market: Option<Vec<String>>,
    ) -> PyResult<Bound<'py, PyAny>> {
        let client = self.client.clone();
        let address = self.address.clone();
        pyo3_async_runtimes::tokio::future_into_py(py, async move {
            ...
            if let Some(v) = market { req = req.market(v); }
            let result = req.send().await.map_err(data_err)?;
            Ok(result.into_iter().map(PyUserValue::from).collect::<Vec<_>>())
        })
    }
    ...
    fn closed_positions<'py>(
        &self, py: Python<'py>,
        market: Option<Vec<String>>, event_id: Option<Vec<String>>,
        title: Option<String>, limit: Option<u32>, offset: Option<u32>,
        sort_by: Option<String>, sort_direction: Option<String>,
    ) -> PyResult<Bound<'py, PyAny>> {
        let client = self.client.clone();
        let address = self.address.clone();
        let sort_by = sort_by.map(|s| parse_closed_position_sort_by(&s)).transpose()?;
        let sort_direction = sort_direction.map(|s| parse_sort_direction(&s)).transpose()?;
        pyo3_async_runtimes::tokio::future_into_py(py, async move {
            ...
            if let Some(v) = sort_direction { req = req.sort_direction(v); }
            let result = req.send().await.map_err(data_err)?;
            Ok(result.into_iter().map(PyClosedPosition::from).collect::<Vec<_>>())
        })
    }
    ...
    fn trades<'py>(
        &self, py: Python<'py>,
        market: Option<Vec<String>>, event_id: Option<Vec<String>>,
        side: Option<String>, taker_only: Option<bool>,
        filter_type: Option<String>, filter_amount: Option<f64>,
        limit: Option<u32>, offset: Option<u32>,
    ) -> PyResult<Bound<'py, PyAny>> {
        let client = self.client.clone();
        let address = self.address.clone();
        let side = side.map(|s| parse_trade_side(&s)).transpose()?;
        let filter_type = filter_type.map(|s| parse_trade_filter_type(&s)).transpose()?;
        pyo3_async_runtimes::tokio::future_into_py(py, async move {
            let mut req = client.user(&address).trades();
            if let Some(v) = market { req = req.market(v); }
            if let Some(v) = event_id { req = req.event_id(v); }
            if let Some(v) = side { req = req.side(v); }
            if let Some(v) = taker_only { req = req.taker_only(v); }
            if let Some(v) = filter_type { req = req.filter_type(v); }
            if let Some(v) = filter_amount { req = req.filter_amount(v); }
            if let Some(v) = limit { req = req.limit(v); }
            if let Some(v) = offset { req = req.offset(v); }
            let result = req.send().await.map_err(data_err)?;
    ...
    fn activity<'py>(
        &self, py: Python<'py>,
        market: Option<Vec<String>>, event_id: Option<Vec<String>>,
        activity_type: Option<Vec<String>>, side: Option<String>,
        start: Option<i64>, end: Option<i64>,
        limit: Option<u32>, offset: Option<u32>,
        sort_by: Option<String>, sort_direction: Option<String>,
    ) -> PyResult<Bound<'py, PyAny>> {
        let client = self.client.clone();
        ...
        let activity_type = activity_type
            .map(|v| v.iter().map(|s| parse_activity_type(s)).collect::<PyResult<Vec<_>>>())
            .transpose()?;
        let side = side.map(|s| parse_trade_side(&s)).transpose()?;
        ...
        pyo3_async_runtimes::tokio::future_into_py(py, async move {
            ...
            if let Some(v) = market { req = req.market(v); }
            if let Some(v) = event_id { req = req.event_id(v); }
            if let Some(v) = activity_type { req = req.activity_type(v); }
            if let Some(v) = side { req = req.side(v); }
            if let Some(v) = start { req = req.start(v); }
            if let Some(v) = end { req = req.end(v); }
            if let Some(v) = limit { req = req.limit(v); }
            if let Some(v) = offset { req = req.offset(v); }
            if let Some(v) = sort_by { req = req.sort_by(v); }
            if let Some(v) = sort_direction { req = req.sort_direction(v); }
    ...
    fn traded<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyAny>> {
        ...
        let address = self.address.clone();
        pyo3_async_runtimes::tokio::future_into_py(py, async move {
            Ok(PyUserTraded::from(client.user(&address).traded().await.map_err(data_err)?))
        })
    }
}
...
impl PyDataApiUserSync {
    ...
    #[pyo3(signature = (*, market=None))]
    fn positions_value(&self, py: Python<'_>, market: Option<Vec<String>>) -> PyResult<Vec<PyUserValue>> {
        let client = self.client.clone();
        let address = self.address.clone();
        py.detach(|| {
            crate::runtime::runtime().block_on(async move {
...
client_ns!(
    ...
    #[allow(clippy::too_many_arguments)]
    fn list(
        user: Option<String>, market: Option<Vec<String>>,
        event_id: Option<Vec<String>>, side: Option<String>,
        taker_only: Option<bool>, filter_type: Option<String>,
        filter_amount: Option<f64>, limit: Option<u32>, offset: Option<u32>,
    ) -> Vec<PyTrade> {
        let mut req = client.trades().list();
        if let Some(v) = user { req = req.user(v); }
        if let Some(v) = market { req = req.market(v); }
        if let Some(v) = event_id { req = req.event_id(v); }
        if let Some(v) = side { req = req.side(parse_trade_side(&v)?); }
        if let Some(v) = taker_only { req = req.taker_only(v); }
        if let Some(v) = filter_type { req = req.filter_type(parse_trade_filter_type(&v)?); }
        if let Some(v) = filter_amount { req = req.filter_amount(v); }
        if let Some(v) = limit { req = req.limit(v); }
        if let Some(v) = offset { req = req.offset(v); }
        let result = req.send().await.map_err(data_err)?;
...
client_ns!(
    ...
    fn list(markets: Vec<String>, limit: Option<u32>, min_balance: Option<u32>) -> Vec<PyMarketHolders> {
        let mut req = client.holders().list(markets);
        if let Some(v) = limit { req = req.limit(v); }
        if let Some(v) = min_balance { req = req.min_balance(v); }
        let result = req.send().await.map_err(data_err)?;
        Ok(result.into_iter().map(PyMarketHolders::from).collect::<Vec<_>>())
    }
);
...
client_ns!(
    ...
    fn get(market: Option<Vec<String>>) -> Vec<PyOpenInterest> {
        ...
        if let Some(v) = market { req = req.market(v); }
        let result = req.send().await.map_err(data_err)?;
        Ok(result.into_iter().map(PyOpenInterest::from).collect::<Vec<_>>())
    }
);
...
client_ns!(
    ...
    fn get(event_id: u64) -> Vec<PyLiveVolume> {
        ...
        Ok(result.into_iter().map(PyLiveVolume::from).collect::<Vec<_>>())
    }
);
...
client_ns!(
    ...
    fn get(
        category: Option<String>, time_period: Option<String>,
        order_by: Option<String>, limit: Option<u32>, offset: Option<u32>,
        user: Option<String>, user_name: Option<String>,
    ) -> Vec<PyTraderRanking> {
        ...
        if let Some(v) = offset { req = req.offset(v); }
        if let Some(v) = user { req = req.user(v); }
        if let Some(v) = user_name { req = req.user_name(v); }
        let result = req.send().await.map_err(data_err)?;
        Ok(result.into_iter().map(PyTraderRanking::from).collect::<Vec<_>>())
    }
);
...
client_ns!(
    ...
    fn leaderboard(
        time_period: Option<String>, limit: Option<u32>, offset: Option<u32>,
        ...
        if let Some(v) = offset { req = req.offset(v); }
        let result = req.send().await.map_err(data_err)?;
        Ok(result.into_iter().map(PyBuilderRanking::from).collect::<Vec<_>>())
    }
    ...
    fn volume(time_period: Option<String>) -> Vec<PyBuilderVolume> {
        ...
        let result = req.send().await.map_err(data_err)?;
        Ok(result.into_iter().map(PyBuilderVolume::from).collect::<Vec<_>>())
    }
);
...
impl PyDataApi {
    ...
    fn user(&self, address: String) -> PyDataApiUser {
        PyDataApiUser { client: self.client.clone(), address }
    }

    fn trades(&self) -> PyDataApiTrades {
        PyDataApiTrades { client: self.client.clone() }
    }

    fn holders(&self) -> PyDataApiHolders {
        PyDataApiHolders { client: self.client.clone() }
    }

    fn open_interest(&self) -> PyDataApiOpenInterest {
        PyDataApiOpenInterest { client: self.client.clone() }
    }

    fn live_volume(&self) -> PyDataApiLiveVolume {
        PyDataApiLiveVolume { client: self.client.clone() }
    }

    fn leaderboard(&self) -> PyDataApiLeaderboard {
        PyDataApiLeaderboard { client: self.client.clone() }
    }

    fn builders(&self) -> PyDataApiBuilders {
        PyDataApiBuilders { client: self.client.clone() }
    }

    fn health(&self) -> PyDataApiHealth {
        PyDataApiHealth { client: self.client.clone() }
    }
}
...
impl PyDataApiSync {
    ...
    fn user(&self, address: String) -> PyDataApiUserSync {
        PyDataApiUserSync { client: self.client.clone(), address }
    }

    fn trades(&self) -> PyDataApiTradesSync {
        PyDataApiTradesSync { client: self.client.clone() }
    }

    fn holders(&self) -> PyDataApiHoldersSync {
        PyDataApiHoldersSync { client: self.client.clone() }
    }

    fn open_interest(&self) -> PyDataApiOpenInterestSync {
        PyDataApiOpenInterestSync { client: self.client.clone() }
    }

    fn live_volume(&self) -> PyDataApiLiveVolumeSync {
        PyDataApiLiveVolumeSync { client: self.client.clone() }
    }

    fn leaderboard(&self) -> PyDataApiLeaderboardSync {
        PyDataApiLeaderboardSync { client: self.client.clone() }
    }

    fn builders(&self) -> PyDataApiBuildersSync {
        PyDataApiBuildersSync { client: self.client.clone() }
    }

    fn health(&self) -> PyDataApiHealthSync {
        PyDataApiHealthSync { client: self.client.clone() }
    }
}
