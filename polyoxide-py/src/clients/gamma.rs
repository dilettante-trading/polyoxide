...
client_ns!(
    ...
    fn get(id: String, include_tag: Option<bool>) -> PyMarket {
        ...
        if let Some(v) = include_tag { req = req.include_tag(v); }
    ...
    fn list(
        limit: Option<u32>, offset: Option<u32>, order: Option<String>,
        ascending: Option<bool>, id: Option<Vec<i64>>, slug: Option<Vec<String>>,
        clob_token_ids: Option<Vec<String>>, condition_ids: Option<Vec<String>>,
        liquidity_num_min: Option<f64>, liquidity_num_max: Option<f64>,
        volume_num_min: Option<f64>, volume_num_max: Option<f64>,
        tag_id: Option<i64>, related_tags: Option<bool>,
        closed: Option<bool>, open: Option<bool>, archived: Option<bool>,
    ) -> Vec<PyMarket> {
        let mut req = client.markets().list();
        if let Some(v) = limit { req = req.limit(v); }
        if let Some(v) = offset { req = req.offset(v); }
        if let Some(v) = order { req = req.order(v); }
        if let Some(v) = ascending { req = req.ascending(v); }
        if let Some(v) = id { req = req.id(v); }
        if let Some(v) = slug { req = req.slug(v); }
        if let Some(v) = clob_token_ids { req = req.clob_token_ids(v); }
        if let Some(v) = condition_ids { req = req.condition_ids(v); }
        if let Some(v) = liquidity_num_min { req = req.liquidity_num_min(v); }
        if let Some(v) = liquidity_num_max { req = req.liquidity_num_max(v); }
        if let Some(v) = volume_num_min { req = req.volume_num_min(v); }
        if let Some(v) = volume_num_max { req = req.volume_num_max(v); }
        if let Some(v) = tag_id { req = req.tag_id(v); }
        if let Some(v) = related_tags { req = req.related_tags(v); }
        if let Some(v) = closed { req = req.closed(v); }
        if let Some(v) = open { req = req.open(v); }
        if let Some(v) = archived { req = req.archived(v); }
...
client_ns!(
    ...
    fn list(
        limit: Option<u32>, offset: Option<u32>, order: Option<String>,
        ascending: Option<bool>, id: Option<Vec<i64>>, slug: Option<Vec<String>>,
        tag_id: Option<i64>, tag_slug: Option<String>,
        active: Option<bool>, archived: Option<bool>,
        featured: Option<bool>, closed: Option<bool>,
        liquidity_min: Option<f64>, liquidity_max: Option<f64>,
        volume_min: Option<f64>, volume_max: Option<f64>,
    ) -> Vec<PyEvent> {
        ...
        if let Some(v) = slug { req = req.slug(v); }
        if let Some(v) = tag_id { req = req.tag_id(v); }
        if let Some(v) = tag_slug { req = req.tag_slug(v); }
        if let Some(v) = active { req = req.active(v); }
        if let Some(v) = archived { req = req.archived(v); }
        if let Some(v) = featured { req = req.featured(v); }
        if let Some(v) = closed { req = req.closed(v); }
        if let Some(v) = liquidity_min { req = req.liquidity_min(v); }
        if let Some(v) = liquidity_max { req = req.liquidity_max(v); }
        if let Some(v) = volume_min { req = req.volume_min(v); }
        if let Some(v) = volume_max { req = req.volume_max(v); }
        let result = req.send().await.map_err(gamma_err)?;
    ...
    fn get(id: String, include_chat: Option<bool>, include_template: Option<bool>) -> PyEvent {
        ...
        if let Some(v) = include_chat { req = req.include_chat(v); }
        if let Some(v) = include_template { req = req.include_template(v); }
        Ok(PyEvent::from(req.send().await.map_err(gamma_err)?))
    }

    #[pyo3(signature = (slug, *, include_chat=None, include_template=None))]
    fn get_by_slug(slug: String, include_chat: Option<bool>, include_template: Option<bool>) -> PyEvent {
    ...
    fn get_related_by_slug(slug: String) -> Vec<PyEvent> {
        let result = client.events().get_related_by_slug(slug).send().await.map_err(gamma_err)?;
        Ok(result.into_iter().map(PyEvent::from).collect::<Vec<_>>())
    }
    ...
    fn tweet_count(id: String) -> PyCountResponse {
        Ok(PyCountResponse::from(client.events().tweet_count(id).send().await.map_err(gamma_err)?))
    }

    #[pyo3(signature = (id,))]
    fn comment_count(id: String) -> PyCountResponse {
        Ok(PyCountResponse::from(client.events().comment_count(id).send().await.map_err(gamma_err)?))
    }
);
...
client_ns!(
    ...
    fn list(
        limit: Option<u32>, offset: Option<u32>, ascending: Option<bool>,
        closed: Option<bool>, slug: Option<Vec<String>>,
        include_chat: Option<bool>, recurrence: Option<String>,
    ) -> Vec<PySeriesData> {
        ...
        if let Some(v) = slug { req = req.slug(v); }
        if let Some(v) = include_chat { req = req.include_chat(v); }
        if let Some(v) = recurrence { req = req.recurrence(v); }
        let result = req.send().await.map_err(gamma_err)?;
        Ok(result.into_iter().map(PySeriesData::from).collect::<Vec<_>>())
    }
...
client_ns!(
    ...
    fn list(
        limit: Option<u32>, offset: Option<u32>, order: Option<String>,
        ascending: Option<bool>, include_template: Option<bool>,
        ...
        if let Some(v) = ascending { req = req.ascending(v); }
        if let Some(v) = include_template { req = req.include_template(v); }
        if let Some(v) = is_carousel { req = req.is_carousel(v); }
        let result = req.send().await.map_err(gamma_err)?;
        Ok(result.into_iter().map(PyTag::from).collect::<Vec<_>>())
    }
    ...
    fn get(id: String) -> PyTag {
        Ok(PyTag::from(client.tags().get(id).send().await.map_err(gamma_err)?))
    }

    #[pyo3(signature = (slug,))]
    fn get_by_slug(slug: String) -> PyTag {
        Ok(PyTag::from(client.tags().get_by_slug(slug).send().await.map_err(gamma_err)?))
    }
    ...
    fn get_related(id: String, omit_empty: Option<bool>, status: Option<String>) -> Vec<PyTag> {
        ...
        if let Some(v) = omit_empty { req = req.omit_empty(v); }
        if let Some(v) = status { req = req.status(v); }
        let result = req.send().await.map_err(gamma_err)?;
        Ok(result.into_iter().map(PyTag::from).collect::<Vec<_>>())
    }

    #[pyo3(signature = (slug, *, omit_empty=None, status=None))]
    fn get_related_by_slug(slug: String, omit_empty: Option<bool>, status: Option<String>) -> Vec<PyTag> {
...
client_ns!(
    ...
    fn list(
        limit: Option<u32>, offset: Option<u32>, order: Option<String>,
        ascending: Option<bool>, parent_entity_type: Option<String>,
        parent_entity_id: Option<i64>, get_positions: Option<bool>,
        ...
        if let Some(v) = order { req = req.order(v); }
        if let Some(v) = ascending { req = req.ascending(v); }
        if let Some(v) = parent_entity_type { req = req.parent_entity_type(v); }
        if let Some(v) = parent_entity_id { req = req.parent_entity_id(v); }
        if let Some(v) = get_positions { req = req.get_positions(v); }
        if let Some(v) = holders_only { req = req.holders_only(v); }
        let result = req.send().await.map_err(gamma_err)?;
    ...
    fn get(id: String) -> PyComment {
        Ok(PyComment::from(client.comments().get(id).send().await.map_err(gamma_err)?))
    }

    #[pyo3(signature = (address,))]
    fn by_user(address: String) -> Vec<PyComment> {
        let result = client.comments().by_user(address).send().await.map_err(gamma_err)?;
        Ok(result.into_iter().map(PyComment::from).collect::<Vec<_>>())
    }
);
...
client_ns!(
    ...
    fn list() -> Vec<PySportMetadata> {
        ...
        Ok(result.into_iter().map(PySportMetadata::from).collect::<Vec<_>>())
    }

    #[pyo3(signature = ())]
    fn market_types() -> Py<PyAny> {
        let result = client.sports().market_types().send().await.map_err(gamma_err)?;
    ...
    fn list_teams(
        limit: Option<u32>, offset: Option<u32>, order: Option<String>,
        ascending: Option<bool>, league: Option<Vec<String>>,
        name: Option<Vec<String>>, abbreviation: Option<Vec<String>>,
    ) -> Vec<PyTeam> {
        ...
        if let Some(v) = order { req = req.order(v); }
        if let Some(v) = ascending { req = req.ascending(v); }
        if let Some(v) = league { req = req.league(v); }
        if let Some(v) = name { req = req.name(v); }
        if let Some(v) = abbreviation { req = req.abbreviation(v); }
        let result = req.send().await.map_err(gamma_err)?;
...
client_ns!(
    ...
    #[allow(clippy::too_many_arguments)]
    fn public_search(
        query: String, limit_per_type: Option<u32>, page: Option<u32>,
        cache: Option<bool>, events_status: Option<String>,
        events_tag: Option<Vec<i64>>, keep_closed_markets: Option<bool>,
        sort: Option<String>, search_tags: Option<bool>,
        search_profiles: Option<bool>,
    ) -> PySearchResponse {
        let mut req = client.search().public_search(query);
        if let Some(v) = limit_per_type { req = req.limit_per_type(v); }
        if let Some(v) = page { req = req.page(v); }
        if let Some(v) = cache { req = req.cache(v); }
        if let Some(v) = events_status { req = req.events_status(v); }
        if let Some(v) = events_tag { req = req.events_tag(v); }
        if let Some(v) = keep_closed_markets { req = req.keep_closed_markets(v); }
        if let Some(v) = sort { req = req.sort(v); }
        if let Some(v) = search_tags { req = req.search_tags(v); }
        if let Some(v) = search_profiles { req = req.search_profiles(v); }
...
client_ns!(
    ...
    fn get(address: String) -> PyUserResponse {
        Ok(PyUserResponse::from(client.user().get(address).send().await.map_err(gamma_err)?))
    }
);
...
impl PyGamma {
    ...
    fn new(
        ...
        if let Some(v) = base_url { builder = builder.base_url(v); }
        if let Some(v) = timeout_ms { builder = builder.timeout_ms(v); }
        if let Some(v) = pool_size { builder = builder.pool_size(v); }
        let client = builder.build().map_err(gamma_err)?;
        Ok(Self { client: Arc::new(client) })
    }

    fn markets(&self) -> PyGammaMarkets {
        PyGammaMarkets { client: self.client.clone() }
    }

    fn events(&self) -> PyGammaEvents {
        PyGammaEvents { client: self.client.clone() }
    }

    fn series(&self) -> PyGammaSeries {
        PyGammaSeries { client: self.client.clone() }
    }

    fn tags(&self) -> PyGammaTags {
        PyGammaTags { client: self.client.clone() }
    }

    fn comments(&self) -> PyGammaComments {
        PyGammaComments { client: self.client.clone() }
    }

    fn sports(&self) -> PyGammaSports {
        PyGammaSports { client: self.client.clone() }
    }

    fn search(&self) -> PyGammaSearch {
        PyGammaSearch { client: self.client.clone() }
    }

    fn user(&self) -> PyGammaUser {
        PyGammaUser { client: self.client.clone() }
    }

    fn health(&self) -> PyGammaHealth {
        PyGammaHealth { client: self.client.clone() }
    }
}
...
impl PyGammaSync {
    ...
    fn markets(&self) -> PyGammaMarketsSync {
        PyGammaMarketsSync { client: self.client.clone() }
    }

    fn events(&self) -> PyGammaEventsSync {
        PyGammaEventsSync { client: self.client.clone() }
    }

    fn series(&self) -> PyGammaSeriesSync {
        PyGammaSeriesSync { client: self.client.clone() }
    }

    fn tags(&self) -> PyGammaTagsSync {
        PyGammaTagsSync { client: self.client.clone() }
    }

    fn comments(&self) -> PyGammaCommentsSync {
        PyGammaCommentsSync { client: self.client.clone() }
    }

    fn sports(&self) -> PyGammaSportsSync {
        PyGammaSportsSync { client: self.client.clone() }
    }

    fn search(&self) -> PyGammaSearchSync {
        PyGammaSearchSync { client: self.client.clone() }
    }

    fn user(&self) -> PyGammaUserSync {
        PyGammaUserSync { client: self.client.clone() }
    }

    fn health(&self) -> PyGammaHealthSync {
        PyGammaHealthSync { client: self.client.clone() }
    }
}
