...
**Retriability** — `ApiError::is_retriable()` (and `ClobError::is_retriable()`) is the canonical classifier for callers' retry policies: true for rate limits, timeouts, connection failures, `425 Too Early`, and 5xx. The crates' *own* retry loop is narrower — `HttpClient::should_retry` only ever retries `429` and `425`.
